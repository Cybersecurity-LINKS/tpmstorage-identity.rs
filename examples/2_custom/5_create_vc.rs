...
async fn main() -> anyhow::Result<()> {
  ...
  println!("Issuer secret manager ready");
  ...
  let alice_storage = examples::tpm_utils::TpmIdentityStorage::new(
    TpmStorage::new(tpm)?,
    KeyIdMemstore::new());

  println!("Alice secret manager ready");

  let (_, alice_document, _): (Address, IotaDocument, String) =
    examples::tpm_utils::create_did(&client, &mut secret_manager_alice, &alice_storage).await?;

  println!("Alice document created");
